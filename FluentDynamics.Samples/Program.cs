...
IOrganizationServiceAsync2 service = ConnectToDataverse();

// Run different query examples
await RunBasicQueryExampleAsync(service);
await RunFilterBuilderExampleAsync(service);
await RunComplexFilterExampleAsync(service);
await RunJoinExampleAsync(service);
await RunPaginationExampleAsync(service);
await RunLinqStyleOperationsExampleAsync(service);

// Advanced examples
await RunAdvancedFilterExampleAsync(service);


/// <summary>
/// Example of connecting to Dataverse
/// </summary>
static IOrganizationServiceAsync2 ConnectToDataverse()
{
    ...
    if ((service as ServiceClient).IsReady)
    {
...
/// Basic query example
/// </summary>
static async Task RunBasicQueryExampleAsync(IOrganizationServiceAsync2 service)
{
    ...
    // Execute the query
    var accounts = await basicQuery.RetrieveMultipleAsync(service);
...
/// </summary>
static async Task RunFilterBuilderExampleAsync(IOrganizationServiceAsync2 service)
{
...
/// </summary>
static async Task RunComplexFilterExampleAsync(IOrganizationServiceAsync2 service)
{
    ...
    try
    {
        ...
        {
            Console.WriteLine($"  - {contact.TryGet<string>("firstname")} " +
                             $"{contact.TryGet<string>("lastname")} " +
                             $"({contact.TryGet<string>("emailaddress1")})");
        }
    }
    catch (Exception ex)
    {
...
/// </summary>
static async Task RunJoinExampleAsync(IOrganizationServiceAsync2 service)
{
    ...
    {
        var value = opportunity.TryGet<Money>("estimatedvalue")?.Value;
        var accountName = opportunity.TryGet<AliasedValue>("account.name")?.Value as string;
        var contactName = opportunity.TryGet<AliasedValue>("contact.fullname")?.Value as string;
...
/// Pagination example
/// </summary>
static async Task RunPaginationExampleAsync(IOrganizationServiceAsync2 service)
{
    ...
    do
    {
        ...
        var pagedResults = await query.RetrieveMultipleAsync(service, pageNumber, pageSize);
        ...
        {
            Console.WriteLine($"    - {account.TryGet<string>("name")}");
        }
...
/// </summary>
static async Task RunLinqStyleOperationsExampleAsync(IOrganizationServiceAsync2 service)
{
...
/// </summary>
static async Task RunAdvancedFilterExampleAsync(IOrganizationServiceAsync2 service)
{
